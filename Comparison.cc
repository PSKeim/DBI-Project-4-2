...
void OrderMaker :: CopyMe (OrderMaker *destOrder) {

	destOrder->numAtts = numAtts;

	for (int i = 0; i < numAtts; i++)
	{
		
		destOrder->whichAtts[i] = whichAtts[i];
		destOrder->whichTypes[i] = whichTypes[i];
...
int CNF :: CreateQueryOrderMaker (OrderMaker &sortorder, OrderMaker &query,
				  OrderMaker &lom) {

  int numMatchedAttrs = 0;
  ...
  for (int i = 0; i < numAnds; i++){
	...
	// If the comparison isn't an equality, continue on
      if (orList[i][j].op != Equals){
	continue;
      }
	// Else if there isn't an operand and a literal, continue
      else if (!((orList[i][j].operand1 == Literal &&
	           orList[i][j].operand2 != Literal) ||
	         (orList[i][j].operand1 != Literal &&
	           orList[i][j].operand2 == Literal))) {
	continue;
      }

	// Get the attribute that isn't the literal value
      attr = orList[i][j].whichAtt1;
      lit = orList[i][j].whichAtt2;
 
      if (orList[i][j].operand1 == Literal){
	attr = orList[i][j].whichAtt2;
	lit = orList[i][j].whichAtt1;
      }

	// If the attribute is the same as that given by the
	// current sort order, then add it to the query OrderMaker
      if (attr == sortorder.whichAtts[soIndex]){

	  // Set the flag
	matchFound = true;

	  // Query OrderMaker has one more attribute
	query.numAtts++;

	  // Set the attribute information for the query OrderMaker
	query.whichAtts[qIndex] = attr;
	query.whichTypes[qIndex] = orList[i][j].attType;

	lom.numAtts++;
	lom.whichAtts[qIndex] = lit;
	lom.whichTypes[qIndex] = orList[i][j].attType;


	  // Increment the current attribute considered
	soIndex++;
	qIndex++;
	...
	numMatchedAttrs++;
	  ...
	  // Attribute matched -- no need to continue searching
	break;
      ...
      // If a match was found, might need to go through
      // loop again
    if (matchFound){
	...
	// Reset the loop if there are still attributes that
	// might get matched
      if (soIndex != sortorder.numAtts){
	i = -1;
      } // end if soIndex

    } // end if matchFound

    // Note that if an attribute was not matched between
    // sortorder and CNF for all subexpressions, the loop will end.
    // The loop over i resets only if (a) an attribute was matched and
    // (b) there are more attributes in sortorder to (potentially)
    // match.

  } // end for i

  return numMatchedAttrs;
