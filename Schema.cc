...
Schema :: ~Schema () {
	  ...
	  // Free the filename - this was created using
	  // strdup(), which uses malloc()
	free(fileName);


	  // The name field for each attribute was
	  // created using strdup(), which uses malloc().
	  // Need to call free() on the name fields, deleting
